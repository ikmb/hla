...
if options.pdf
    ...
    # #<CSV::Row "Sample name":"4057835582" "Locus":"DQB1" "No. of reads":"2877" "Read Length":"229" "Insert size":"537" "Mappability %":"99" "Read Depth - Median":"173" "Number of question mark positions":"0" "Phasing regions":"1" "Possible genotypes":">10" "Exon / Intron mismatches":"[Ex] 0 [In] 6" "Typing result":"DQB1*02:01:01:01, DQB1*05:01:01:03">
        
    csv.each do |row|
	patient = row["Sample name"]
	locus = row["Locus"]
	calls = row["Typing result"].split(",").map {|c| trim_allele(c.strip.split("*")[-1],precision) }
    ...
    abort "Did not provide a valid input format (--pdf or --csv)"
